...
/**
 * WireMock Configuration for OpenApi API
 * Auto-generated from OpenAPI specification: open-api-spec.yaml
 * Generated on: 2025-09-11 11:39:36
 * 
 * Usage in tests:
 * @SpringBootTest
 * @Import(OpenApiWireMockConfig.class)
 * class YourTest {
 *     @Autowired
 *     private String open_apiApiBaseUrl;
 * }
 */
@TestConfiguration
public class OpenApiWireMockConfig {
    ...
    @Bean
    @Primary
    public String open_apiApiBaseUrl() {
        return "http://localhost:" + WIREMOCK_PORT;
    }
    
    @Bean
    public WireMockServer open_apiWireMockServer() {
    ...
    private void setupDefaultStubs() {
        ...
        stubFor(get(urlPathEqualTo("/health"))
            .willReturn(aResponse()
                ...
                .withBody("{\"status\": \"UP\", \"service\": \"open_api\"}")));
        ...
        stubFor(any(urlMatching(".*"))
            ...
            .willReturn(aResponse()
                .withStatus(404)
                .withHeader("Content-Type", "application/json")
                .withBody("{\"error\": \"Endpoint not found\", \"service\": \"open_api\"}")));
    }
